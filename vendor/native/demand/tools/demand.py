...
from ml.config import STAFF,WEATHER,ALIASES
...
def predict_demand(input_data, *, request_id, config:ServiceConfig, availability=None,weather_record_id=None):
    ...
    allowed=set(STAFF)|{'date','weekday','menu'}
    allowed|={k for k,v in ALIASES.items() if v in allowed}
    if set(input_data)-allowed:
        raise DemandError('Unsupported input fields: '+str(sorted(set(input_data)-allowed)))
    if availability is not None and not isinstance(availability,dict):
        raise AvailabilityError('availability must be a per-field object')
    key=request_key(request_id)
    # The original policy is stored with the response. Server policy/model
    # changes must not turn an identical client retry into a new prediction.
    fingerprint=digest(dict(input_data=input_data,availability=availability,weather_record_id=weather_record_id))
