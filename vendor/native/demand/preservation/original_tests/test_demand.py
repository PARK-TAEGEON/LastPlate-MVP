...
def test_predict_record_deduplicate_snapshot(sandbox, data):
    root, r, meta = sandbox
    p = predict_demand(input_row(data.iloc[100]), r.path)
    assert p['model_version'] == meta['model_version']
    assert isinstance(p['prediction'], int) and p['prediction'] >= 0
    assert 'lower' not in p and 'upper' not in p
    path = root / 'data/operational_history.csv'
    row = record_actual_result(p, 900, history_path=path, model_dir=r.path, weather={'rain': True})
    assert row['prediction_error'] == 900-p['prediction']
    assert json.loads(row['input_json']) == p['input_data']
    assert len(read_history(path)) == 1
    with pytest.raises(ValueError, match='already exists'):
        record_actual_result(p, 900, history_path=path, model_dir=r.path)
    p['prediction'] += 1
...
def test_retrain_insufficient(sandbox):
    root, _, _ = sandbox
    assert batch_retrain(root)['status'] == 'insufficient_new_data'

def test_weekly_not_due_and_invalid_gate(sandbox):
    root, _, _ = sandbox
    assert batch_retrain(root, trigger='weekly')['status'] == 'not_due'
    with pytest.raises(ValueError):
        batch_retrain(root, holdout_size=0)

@pytest.mark.parametrize('candidate_value,expected', [(900, 'promoted'), (100000, 'kept_incumbent')])
def test_batch_gate_promotion_and_rejection(sandbox, data, monkeypatch, candidate_value, expected):
    root, r, meta = sandbox
    populate_history(root, r, data)
    seen = []
    def fit(d, kind, group):
        seen.append(d.copy())
        return dummy_bundle(d, candidate_value)
    monkeypatch.setattr('ml.retrain.fit_bundle', fit)
    result = batch_retrain(root)
    assert result['status'] == expected
    assert seen[0].date.max() < pd.Timestamp(result['gate_period']['start'])
    assert pd.Timestamp(meta['trained_through']) < pd.Timestamp(result['gate_period']['start'])
    if expected == 'promoted':
        assert len(seen) == 2 and len(seen[1]) == 130
        assert r.current_version() != meta['model_version']
        assert (r.path / 'archive' / meta['model_version']).exists()
    else:
        assert len(seen) == 1 and r.current_version() == meta['model_version']
    assert batch_retrain(root)['status'] == 'insufficient_new_data'
    assert batch_retrain(root, trigger='manual')['status'] == 'insufficient_holdout'
