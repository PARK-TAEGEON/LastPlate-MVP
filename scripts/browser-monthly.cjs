...
(async()=>{
 ...
 await fetch(url+'/__e2e_fail_next_demand',{method:'POST'});
 ...
 assert.match(await page.locator('[data-calendar-date="2026-09-19"]').innerText(),/계산 미완료 · 다시 시도/);
 assert.match(await page.locator('#month-summary').innerText(),/계산 완료 1일 · 계산 미완료 1일/);
 assert.equal(await page.locator('#day-apply').innerText(),'계산 다시 시도');
 assert.doesNotMatch(await page.locator('.next-action').innerText(),/재료 부족 · 유통기한/);
 const partialMonth=await(await fetch(url+base+'?site_id=DEMO-LH')).json();checks.push('saved partial calculation shown as incomplete with real retry');
 assert.match(await page.locator('[data-calendar-date="2026-09-21"]').innerText(),/계획 생성됨|확인 필요/);checks.push('batch partial failure retains successful dates');
 ...
 assert.notEqual(oldDay.plan_id,partialMonth.days[0].plan_id);assert.equal(old.days[2].plan_id,partialMonth.days[2].plan_id);assert.equal(first.calculation_complete,true);assert.doesNotMatch(await page.locator('#plan-summary').innerText(),/계산 미완료/);checks.push('retry replaces failed calculation and preserves completed dates');
