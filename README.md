# 라플 / LastPlate MVP 1.3.0 — 월간 급식 운영 계획

**1.3.0 식수·조리량 확정 / 데이터 연결 시연:** 날짜별 직접 입력 식수로 서버가 조리량과 재료량을 재계산하고, 운영자의 명시적 확정 기록을 저장합니다. 식단·인원·자료 변경은 기존 확정을 무효화합니다. 시연 사업장의 확정은 “시연 조리량 확정”으로 구분합니다. 계산 미완료·저장 실패·차단 조건·용량 초과는 확정할 수 없습니다. 경고 확인은 자료 검증이나 위험 해제를 뜻하지 않습니다.

외부 데이터 연결 영역에는 레시피·영양·가격 추이·공급 위험을 각 1~2건씩 선택 적용하는 시연 제공자가 있습니다. API 실응답이 아닌 명시적 DEMO 자료이며 실제 네트워크 연결과 인증키 읽기는 아직 없습니다. `backend/application/data_examples.py`의 검증된 입력 패치를 실제 제공자로 교체할 수 있습니다. [연결 구조와 공식 명세](docs/data-providers.md)를 참고하세요.

**1.2.3 재고 누락 처리 수정:** 재고가 없는 품목 때문에 조리 단계가 멈췄을 때 빈 급식 용량 값을 비교해 HTTP 500이 발생하던 표시 오류를 수정했습니다. 누락 품목을 안내하고 입력 보완 후 재계산할 수 있습니다. 재고 다운로드 양식에도 밥·국 재료를 포함했습니다. 미확인 재고를 0이나 임의 수량으로 바꾸지는 않습니다.
...
Git으로 받은 경우 `git pull --ff-only`, ZIP으로 받은 경우 최신 소스 파일로 갱신하세요. 실행 중인 서버를 종료한 뒤 다시 켜야 수정이 적용됩니다. 실패가 반복되면 같은 Python 환경에서 `python scripts/check-runtime.py`를 실행하면 실패 단계와 원인을 터미널에 출력합니다. 이 점검은 임시 DB만 사용합니다.
...
7. Agent의 기타 사유 해석과 메뉴 권고를 수용하거나 반영하지 않기로 기록합니다. 메뉴 수용 후 서버가 다시 계산합니다. **운영 식수**를 입력하고 **변경사항 반영**으로 재계산한 다음 경고 확인을 체크하고 **조리량 확정**을 누르세요. 확정 인원·수량·시각을 저장합니다. 자동 발주는 수행하지 않습니다.
